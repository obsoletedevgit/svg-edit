...
class SvgEditorProvider {
  static register(context) {
    ...
    return vscode.window.registerCustomEditorProvider('svgEdit.editor', provider);
  }
  ...
  getHtml(webview) {
    ...
    const cmCss = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'media/codemirror/lib/codemirror.css')
    );

    const cmJs = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'media/codemirror/lib/codemirror.js')
    );

    const xmlMode = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'media/codemirror/mode/xml/xml.js')
    );
    ...
    return `
      ...
      <body>
        ...
        <script>
          ...
          const editorContainer = editorDiv;
          ...
          const editor = window.createEditor(editorDiv, '', text => {
          ...
          document.getElementById('toggle-preview').addEventListener('click', () => {
            if(previewSvg.style.display==='none'){ previewSvg.style.display='block'; updateTransform(); }
            else { previewSvg.style.display='none'; }
          });
        </script>
      </body>
      </html>
